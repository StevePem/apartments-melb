...
removeBadModes <- function(df, mode, bad_nodes) {
  ...
  df <- df %>%
    ...
    mutate(modes = ifelse(delete == T, 
                          gsub(paste0(mode, ',|', mode, '|,', mode), '', modes), 
                          modes)) %>%
...
largestDirectedNetworkSubgraph <- function(networkList, network_modes) {
  # networkList <- networkMode
  # network_modes <- "car,bike"
  ...
  for (mode in network_modes) {
    ...
    links_directed <- rbind(
      ...
      traversable_links %>% filter(is_oneway == 0) %>% 
    ...
    bad_nodes <- union(
      ...
      undirected_clusters %>% 
        filter(cluster_id %in% c(unreachable_clusters,bad_components)) %>%
        pull(node_id)
...
largestNetworkSubgraph <- function(networkList, network_modes){
  # networkList <- networkMode
  # network_modes <- "walk"
  nodes_df <- networkList[[1]]
  links_df <- networkList[[2]]
