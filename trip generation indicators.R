...
indicators <- function() {
  ...
  # for (i in 2:3) {
    ...
    ## 4.1 skip Palmerston North (because no GTFS) ----
    if (city == "Palmerston North") {
      print(paste(Sys.time(), "| Skipping Palmerston North as there is no GTFS"))
      next
    }
    
    ## 4.2 pedestrian network for city, and graph ----
    ...
    ## 4.3. developments in the city, with nearest nodes ----
    ...
    for (j in seq_along(gtfs.dates)) {
      ...
      ### 4.5.2 stops with modes and nearest nodes ----
      ...
      ### 4.5.3 developments for the gtfs date ----
      developments.date <- developments %>% 
        filter(GTFS == gtfs.date)
        
      ### 4.5.4 loop to process developments ----
      for (k in 1:nrow(developments.date)) {
        ...
        if (nrow(stops) > 0) {
          ...
          #### 4.5.4.1 distance matrix from development to stops ----
          dist <- distances(g, v = development.node, to = stop.nodes) %>%
            as.data.frame()
          
          #### 4.5.4.2 distance to closest stop of each mode ----
          ...
          if (nrow(stops.800m) > 0) {
            ...
            for (l in seq_along(survey.dates)) {
              ...
              ##### 4.5.4.5.1 service id's for the relevant date ----
              services <- calendar %>%
                filter(start_date <= survey.date & end_date >= survey.date &
                         !!sym(survey.day) == 1) %>%
                .$service_id
              
              ##### 4.5.4.5.2 adjust for exceptions in calendar_dates ----
              ...
              ##### 4.5.4.5.3 trips for the relevant services ----
              trips <- gtfs$trips %>%
                filter(service_id %in% services) %>%
                .$trip_id
              
              ##### 4.5.4.5.4 stop times for the relevant trips and stops ----
              ...
              ##### 4.5.4.5.5 tally trips for each mode ----
            ...
            #### 4.5.4.6 average counts where multiple dates ---- 
