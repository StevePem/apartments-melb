...
get.bus.vol <- function(route.stop.location, # folder containing daily route and stop .gz files
  ...
  #   route.stop.location = "../Tables/bus monthly stops and routes/"
  ...
  ## 2 For each month, find the  monthly volumes
  ## -------------------------------------#
  ...
  # loop to find monthly volumes for each month, and combine as monthly volumes
  output.list <- 
    foreach(j = 1:length(bus.months),
            ...
            .options.snow = opts) %dopar% {
              ...
              # paths to monthly details
              month.file.path <- route_stop_file_paths[str_detect(route_stop_file_paths, bus.months[j])]
              
              # read in the monthly details
              monthly.details <- read_csv(month.file.path, show_col_types = FALSE)
              ...
              # find monthly volumes by route
              monthly.totals <- monthly.details %>%
                group_by(route_short_name) %>%
                summarise(count = n_distinct(trip_id, date)) %>%
                ungroup() %>%
