...
configureNetwork <- function(INPUTSQLITE,
  ...
  # join OSM highway tags to edges
  osm_highway <- st_read(INPUTSQLITE, layer="osm_metadata", quiet = T) %>%
    filter(osm_id %in% networkInput[[2]]$osm_id) %>%
    dplyr::select(osm_id, highway)
  
  networkInput[[2]] <- networkInput[[2]] %>%
    inner_join(osm_highway, by = "osm_id")
  
  # test <- networkInput[[2]] %>% filter(is.na(highway))  # 0 obs, as intended
  
  # omit motorways
  networkInput[[2]] <- networkInput[[2]] %>%
    filter(!(highway %in% c("motorway", "motorway_link")))
  networkInput[[1]] <- networkInput[[1]] %>%
    filter(id %in% networkInput[[1]]$from_id | id %in% networkInput[[2]]$to_id)
