...
# Old version, that finds the current tram stops for tram routes, including
# where split - not correct, because need to find past tram stops as well,
# and filter by route for 'corridor_own_service' analysis

# Required functions are in Part B Option 3 of 'analysis.R', which invokes
# this script
...
for (i in 1:nrow(multipart.corridors)) {
  ...
  # update the stop list for the part 
  for (j in 1:nrow(corridor)) {
    replacement.corridor.stops <- tram.stops %>%
      # confine to the stops from the original full route
      mutate(stop_id = as.character(stop_id)) %>%
      filter(stop_id %in% str_split(corridor[j, "stops"][[1]], ", ")[[1]]) %>%
      # select only if in the new part
      filter(st_intersects(., corridor[j, ], sparse = FALSE)) 
    
    corridor[j, "stops"] <- paste(replacement.corridor.stops$stop_id %>% 
                                    unique() %>%
                                    sort(), 
                                  collapse = ", ")
  }
